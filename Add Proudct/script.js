...
function populateColorsAndSizes(product) {
    // الصور مع الألوان
    const colorsList = document.getElementById('colorsList');
    // if (colorsList && product.images) {
    //     colorsList.innerHTML = ''; // مسح القديم
    //     product.images.forEach((img, index) => {
    //         const div = document.createElement('div');
    //         div.className = 'option-item';
    //         div.innerHTML = `
    //             <input type="color" value="${img.color}" disabled>
    //             <img src="${img.url}" alt="صورة ${img.color}" style="width:50px;height:50px;">
    //         `;
    ...
    // if (sizesList && product.sizes) {
    //     sizesList.innerHTML = ''; // مسح القديم
